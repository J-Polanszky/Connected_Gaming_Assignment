...
namespace UnityChess
{
    ...
    public class Game
    {
        ...
        public bool TryExecuteMove(Movement move, out Piece capturedPieceEvent, bool preValidated = false)
        {
            // This has to be used due to how the chess lib was designed. If time permitted, a partial rewrite would have gone
            // a long way in making all of this cleaner
            Movement validatedMove = move;
            if (!preValidated)
            {
                if (!TryGetLegalMove(move.Start, move.End, out Movement validatedMoveReturn))
                {
                    capturedPieceEvent = null;
                    return false;
                }
                validatedMove = validatedMoveReturn;
            }
            ...
            HalfMove halfMove = new HalfMove(boardBeforeMove[validatedMove.Start], validatedMove, capturedPiece,
                causedCheck);
            GameConditions resultingGameConditions =
                conditionsBeforeMove.CalculateEndingConditions(boardBeforeMove, halfMove);
        ...
        public bool TryGetLegalMove(Square startSquare, Square endSquare, out Movement move)
        {
            ...
            return BoardTimeline.TryGetCurrent(out Board currentBoard)
                   && LegalMovesTimeline.TryGetCurrent(
                       out Dictionary<Piece, Dictionary<(Square, Square), Movement>> currentLegalMoves)
                   && currentBoard[startSquare] is { } movingPiece
                   && currentLegalMoves.TryGetValue(movingPiece,
        ...
        public bool TryGetLegalMovesForPiece(Piece movingPiece, out ICollection<Movement> legalMoves)
        {
            ...
            if (movingPiece != null
                && LegalMovesTimeline.TryGetCurrent(
                    out Dictionary<Piece, Dictionary<(Square, Square), Movement>> legalMovesByPiece)
                && legalMovesByPiece.TryGetValue(movingPiece,
                    out Dictionary<(Square, Square), Movement> movesByStartEndSquares)
        ...
        internal static int GetNumLegalMoves(
            Dictionary<Piece, Dictionary<(Square, Square), Movement>> legalMovesByPiece)
        {
            int result = 0;
